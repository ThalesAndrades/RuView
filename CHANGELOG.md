...
### Added
- **Adaptive (self-calibrating) presence threshold — safe by construction.** Presence flipped on a single hardcoded `sm > 0.03`, which over-triggers in an electrically noisy room (a flickering empty room reads as occupied). The threshold is now a margin above the *learned empty-room `sm` noise floor* (an EMA updated only while classified **absent**, so a real occupant's motion can never inflate it), **floored at the proven `0.03`**: `threshold = max(0.03, noise_floor × 2.5)`. In a quiet room the floor governs → behavior is byte-for-byte identical to before; in a noisy room the threshold rises to reject the chatter. It can never drop below `0.03`, so presence is **provably never more trigger-happy than today** (same one-directional-safety guarantee as the #803 count fix). Applied to both the aggregate (`smooth_and_classify`) and per-node (`smooth_and_classify_node`) paths via a pure `adaptive_presence_threshold` helper; `presence_noise_floor` added to `AppStateInner` + `NodeState`. 3 unit tests (quiet→floor, noisy→raised, EMA convergence never below floor).
