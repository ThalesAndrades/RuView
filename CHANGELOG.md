...
### Added
- **Field-validation harness P2 — Tier-B reference sensors + vitals Bland–Altman (`aether-arena/validation/`).** Extends the P1 harness so ground truth no longer needs a human pressing `mark.py` — the bottleneck that made the power-calc targets (≈50 overnight bed-exits, ≈75 presence samples per cell) impractical. New `reference.py` converts independent reference-sensor logs into the *same* artifacts the scorer reads, each sensor an oracle on **its own** log (never derived from the system under test): a **bed load-cell** (`t,weight`/`occupied` CSV) → `bed_exit` point events on the occupied→empty edge + bed presence; a **PIR/contact** (`t,motion`) → `present`/`absent` toggles; a **Polar H10** (`t,hr`) → an HR `vitalsref.jsonl` oracle. Inputs accept epoch-seconds / epoch-ms / ISO-8601 and named-header or positional CSV; outputs append so several sensors share one `groundtruth.jsonl`. `score.py` gained `--vitals-ref`: it pairs each reference HR/BR sample with the nearest system `vital_signs` reading within a window and reports a **Bland–Altman** agreement — bias (mean system−reference), **95% limits of agreement** (bias ± 1.96·SD), MAE, and the within-±N-BPM rate with a Wilson CI. Verified end-to-end on synthetic fixtures (recovered an injected +1.5 BPM HR bias as 1.61, within-±5 0.983; bed-exit falling-edge detected at the exact transition); P1 scoring is unchanged when `--vitals-ref` is omitted. Pure Python 3 stdlib. ADR-151 P3 (live MQTT reference tap, multi-night witness bundles) remains the follow-up.
- **`scenario_demo` example — see the elderly-care safety arc with zero hardware.** `cargo run -p wifi-densepose-sensing-server --example scenario_demo --no-default-features` replays a scripted "night in a resident's room" through the **real** `SemanticBus` FSMs, fast-forwarding time via each snapshot's `since_start`/time-of-day so a 60-minute inactivity gate compresses to one line. Prints the primitives flipping in order — `room_active → someone_sleeping → no_movement → elderly_inactivity_anomaly → bed_exit` (+ `multi_room_transition`) with their explainable `reason` tags — exactly the Home-Assistant entities a caregiver would watch. The fastest way to *see* the product before investing in an ESP32; honest about being a pipeline demo on scripted inputs, not a real-world accuracy claim.
- **Field-validation harness — ADR-151 P1 (`aether-arena/validation/`).** The first tooling to measure the semantic layer's *accuracy against ground truth* instead of trusting demos. Pure Python 3 stdlib (incl. a minimal WebSocket client — no `pip install`): `record.py` logs the live `/ws/sensing` stream (`sensing_update` + `semantic_event`) to JSONL; `mark.py` is a Tier-A ground-truth marker (`present`/`absent` toggles + point events like `bed_exit`); `score.py` aligns them on wall-clock time and emits `report.md` + `metrics.json` — presence confusion (precision/recall/F1/false-alarm-rate/onset-latency with **Wilson 95% CIs**) and event matching (TP/FP/FN + latency). `protocol.md` covers clocks (NTP/sync-clap), a session script, a sample-size calc, and the ADR-150 honesty rule (report held-out / cross-subject / cross-environment separately). Verified end-to-end against a `--source simulate` server. This is the artifact that turns the working pipeline into *defensible accuracy*.
- **Node→zone model unlocks the three zone-gated semantic primitives (ADR-115 §3.12).** `bed_exit`, `bathroom_occupied`, and `multi_room_transition` were fully implemented + tested but dormant because nothing populated `RawSnapshot.active_zones` — the `--semantic-zones-file` flag existed only on the unused lib `cli::Args` with no loader (the #872 dead-wiring pattern). New `semantic::zones::ZoneMap` loads a **zone-centric** JSON schema — `{ "zones": [ { "name": "bedroom", "nodes": ["1"], "tags": ["bed"] }, … ] }` — mapping ESP32 `node_id`s to named zones with tags. `build_semantic_snapshot` now resolves `active_zones` per tick from which nodes currently report presence (the per-node `node_features` breakdown, **excluding stale nodes** so a frozen offline node can't keep a zone "occupied") and `bed_zones` from the static `bed` tag; the `bathroom` primitive matches a zone named after its tag. Added `--semantic-zones-file` to the binary's own `Args` (a malformed file is logged and ignored — the 7 zone-free primitives keep running — rather than failing startup). With a map, an ILPI/elderly-care pilot now gets overnight **bed-exit** and **bathroom-occupancy** events in Home Assistant. Two robustness fixes so zones work on real hardware: (1) **edge-vitals-only nodes now drive zone presence** — a mmWave node (C6 + MR60BHA2, the best bathroom/bed presence sensor) never ran the CSI debounce, so its `current_motion_level` stayed `"absent"` and its zone never activated; presence is now set straight from the firmware-debounced `vitals.presence/motion`. (2) The semantic loop now **skips re-ticking an unchanged `latest_update`** (tracking its `tick`), so a stalled non-ESP32 source can't falsely accumulate duration-based primitives — generalizing the `:offline` guard. 13 new tests (`ZoneMap` parse/resolution/case-insensitivity/malformed-input + per-node + stale-node snapshot resolution). 618 default / 629 `--features mqtt` sensing-server tests pass.
- **Semantic primitives now publish as first-class Home-Assistant MQTT entities (ADR-115 §3.12).** The `SemanticBus` wiring landed the inference layer on the WS stream, but the events never reached Home Assistant — the publisher only understood `VitalsSnapshot`, so a caregiver's HA dashboard couldn't see `binary_sensor.<node>_elderly_inactivity_anomaly`, `no_movement`, `someone_sleeping`, etc. The full discovery/state/privacy/Matter scaffolding for all 10 primitives already existed (`EntityKind`, `DiscoveryBuilder::build`, `StateEncoder`, rate limiter, `is_biometric` clearance) but had **no live routing** from the bus — exactly the cross-protocol channel the `mqtt` module's own docs describe (*"the semantic layer emits primitive state changes onto a `tokio::broadcast` channel that this module also subscribes to"*). This adds that channel: `semantic_tick_task` now feeds a typed `broadcast::Sender<SemanticEvent>`; the MQTT publisher gained a second `select!` arm that maps `SemanticKind → EntityKind`, lazily registers the device, encodes each `PrimitiveState` (boolean → ON/OFF, scalar → `{"score":…}`, event → event payload) via the existing `StateEncoder`, and publishes. The **7 zone-free primitives** (`someone_sleeping`, `possible_distress`, `room_active`, `elderly_inactivity_anomaly`, `meeting_in_progress`, `fall_risk_elevated`, `no_movement`) now surface live in HA; semantic states publish even under `--privacy-mode` (they're inferred, not biometric). The 3 zone-gated ones stay dormant pending the node→zone model. Room-level semantics publish under the base device (per-node semantic routing is tracked follow-up). The MQTT path publishes **independently of WebSocket subscribers** — an HA-only deployment (no browser dashboard) still gets every event — and a kind/state component mismatch is logged rather than silently dropped. New `StateEncoder::scalar` + a broker-level e2e integration test; publisher `spawn`/`run` take the new receiver. 617 sensing-server tests pass with `--features mqtt` (606 default).

### Fixed
- **Learned semantic baselines now survive restarts — a reboot no longer mis-fires the safety gates.** Two primitives learn long-lived baselines: `ElderlyInactivityAnomaly::longest_idle` (the inactivity reference the `2×` anomaly multiplier is applied against) and `PossibleDistress`'s rolling resting-HR baseline. Both were **uptime-only** — every restart reset the inactivity baseline to the 30-min floor, which *drops the firing threshold* and spams false "elderly inactivity" anomalies for hours (a 24/7 elderly-care product reboots: power blips, updates, crashes). New `semantic::persistence` module persists just those two *learned scalars* (never the session-relative timers, which correctly reset) to `<data_dir>/semantic_baselines.json`; `SemanticBus::snapshot`/`restore` capture/apply them, `semantic_tick_task` restores on startup and flushes every 5 min. `restore` floors the inactivity baseline at the 30-min safety minimum so a stale/corrupt file can't make the anomaly fire too eagerly; a missing file is a cold start while a corrupt/unreadable one is logged (not silently ignored). 5 tests (persistence round-trip / absent-file / partial-JSON + bus snapshot-restore + floor guard).
- **Semantic inference bus now actually runs — the advertised "primary product surface" was dead-wired.** The `semantic::SemanticBus` (ADR-115 §3.12 — the 10 HA-MIND primitives: `someone_sleeping`, `possible_distress`, `room_active`, `elderly_inactivity_anomaly`, `meeting_in_progress`, `bathroom_occupied`, `fall_risk_elevated`, `bed_exit`, `no_movement`, `multi_room_transition`) was fully implemented and unit-tested, but `SemanticBus::new`/`bus.tick()` were called **only in `#[cfg(test)]`** — `main.rs` never instantiated or ticked it. So despite `--semantic` defaulting **on** ("primitives are the primary product surface"), the runtime emitted nothing — the same dead-wiring class as #872 (MQTT) and #803 (person count). New `semantic_tick_task` drives the bus from the same broadcast tap the MQTT bridge uses: it projects each live `SensingUpdate` onto a `RawSnapshot` (`build_semantic_snapshot`) and surfaces emitted primitives on the WS stream as `{"type":"semantic_event", "primitive":…, "state":…, "reason":[…]}`, reusing the existing `tx` broadcast (purely additive — no new routes, no change to the `sensing_update` payload). The seven zone-free primitives run fully; the three zone-gated ones (`bed_exit`/`bathroom`/`multi_room`) stay dormant by design until the node→zone model (`--semantic-zones-file`, currently a parsed-but-unconsumed flag) is resolved. Routing events to first-class MQTT/Matter entities + the zone model are the tracked follow-ups. The task skips ticking on a stale source (`effective_source()` `…:offline`) so an ESP32 disconnect can't manufacture false `someone_sleeping`/`no_movement`/inactivity events from a frozen frame, runs at a ~1 Hz floor, and biases unknown `motion_level` labels toward "moving" (never silently "still"). 5 unit tests cover the projection, the `motion_level`→scalar mapping (gated so `present_still` reads as "still" to the elderly-anomaly threshold), and the event envelope.
